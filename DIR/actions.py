...
class ActionTellTime(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        current_place = next(tracker.get_last_event_for("place"), None)
        utc = arrow.utcnow()
...
class ActionTimeDifference(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
            tracker: Tracker,
            domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        timezone_to = next(tracker.get_last_event_for("place"), None)
        ...
        msg = f"There is a {min(diff_hours, 24-diff_hours)}J time difference."
        dispatcher.utter_message(text=msg)
